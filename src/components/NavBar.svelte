...
<header class="sticky flex py-4 items-center bg-slate-700 font-semibold text-4xl text-white border-b border-white justify-between">
    <button on:click={() => {window.location.href='/'}} class="ml-4 text-center text-orange-500 text-4xl italic">Quantitative Traders At Virginia</button>
    {#if $authStore.currentUser}
        <!-- Make sure this only appears if uid is a certain value-->
        {#if $authStore.currentUser.uid === "66Q8I5XwY0TChOmSUWoYM5nEaB32"}
            <button on:click={() => showModal = "create"} class="mr-4 text-lg border p-2 rounded-md">Create game</button>
            <button on:click={start} class="mr-4 text-lg border p-2 rounded-md">Start game</button>
        ...
        <button on:click={() => {window.location.href='/dashboard'}} class="mr-4 text-lg border p-2 rounded-md">{email}</button>
    {:else}
